...
function createLine(startPoint, endPoint, iterations) {
  ...
  //get square boundries
  


  //calculate intersection of square and line
 ...
 //function for creating all folllowing lines (for-loop?)
 //function nextLines{
  
    //Calculate start point on third and following lines
    //function nextStartPoints(linePositions) {

      // Extracting start and end points of the line from the linePositions array
      // using newStartPoint and newEndPoint
   ...
   //calculate nextAngle based on last angle
   //function nextAngle {

      //const nextAngle = newAngle + Math.PI / 2; //for folling new angle = next angle

    //}

    // Calculate the end point of the new line based on the new start point and angle
    //const newEndPoint = new THREE.Vector3(
      //nextStartPoint.x.x + nextLineLength * Math.cos(nextAngle),
      //nextStartPoint.y + nextLineLength * Math.sin(nextAngle),
      //0
    //);

    // Create the next line geometry
    //const nextLineGeometry = new THREE.BufferGeometry().setFromPoints([randomPoint, newEndPoint]);
    //const nextLineMaterial = new THREE.LineBasicMaterial({ color: 0x000000 });
    //const nextLine = new THREE.Line(nextLineGeometry, nextLineMaterial);
